...
struct emu20kx_card_s
{
 ...
 unsigned int  rsr; // reference sampling rate (44100 or 48000)
 unsigned int  last_rsr_cfg;
 ...
 unsigned int  dac_output_freq; // DAC is set to this, other freqs are converted to this by card
...
/////////////////////////////////////////////////////////////////////////////
{
	outpd(card->iobase + 0x0,reg);
	return ((uint32_t)inpd(card->iobase + 0x4));
}
...
{
	outpd(card->iobase + 0x0,reg);
	outpd(card->iobase + 0x4,data);
}

static inline uint32_t hw_read_pci(struct emu20kx_card_s *card,uint32_t reg)
////////////////////////////////////////////////////////////////////////////
{
	outpd(card->iobase + 0x10,reg);
	return ((uint32_t)inpd(card->iobase + 0x14));
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////
{
	...
	if(aui->freq_card < 3000)
		aui->freq_card = 3000;
	else if((aui->freq_card > 96000) && (aui->chan_card > 2) )
		aui->freq_card = 96000;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
{
	...
	if (!MDma_alloc_cardmem( &card->dm, EMU20KX_MAXPAGES*sizeof(uint32_t)// virtualpage
...
static int set_timer_tick(struct emu20kx_card_s *card, unsigned int ticks)
//////////////////////////////////////////////////////////////////////////
{
	if (ticks)
		ticks |= TIMR_IE | TIMR_IP;
	hw_write_20kx(card, TIMR, ticks);
	return 0;
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    dbgprintf(("emu20kx_prepare_playback: exit\n"));
}
...
//////////////////////////////////////////////////////
{
	...
	if(card){
		if(card->iobase){
			...
			//dpmi_unmap_physical_memory(card->iobase);
		}
...
//////////////////////////////////////////////////////////////////
{
	...
	dbgprintf(("emu20kx_getbufpos:%5d dmasize:%5d\n",bufpos,aui->card_dmasize));
