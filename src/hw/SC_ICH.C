...
#define ICH_PO_CIV    0x14  // CIV - Current Index Value (ro; 8-bit, bits 5-7=0)
#define ICH_PO_LVI    0x15  // LVI - Last Valid Index (rw; 8-bit, bits 5-7=0)
#define ICH_PO_SR     0x16  // SR - Status (ro,r/wc; 16-bit, bits 5-15 rsvd)
#define ICH_PO_PICB   0x18  // PICB - Position In Current Buffer (ro; 16-bit; remaining, not processed pos)
#define ICH_PO_PIV    0x1a  // PIV - Prefetched Index Value (ro; 8-bit; bits 5-7=0, not used)
...
#define readpicb(card) (card->device_type == DEVICE_SIS ) ? ich_read_16(card, ICH_PO_PICB_SIS ) : ich_read_16(card, ICH_PO_PICB ) * 2
...
struct bd_s {
    ...
    uint16_t size; /* size in samples */
    uint16_t flags; /* bits 0-13: rsvd; bit 14: underrun policy; bit 15: IOC */
};
...
/////////////////////////////////////////////////////////////////////////////////
{
	...
	for ( retry = ICH_DEFAULT_RETRY; retry; retry-- ) {
		if( ich_read_32(card,ICH_GBL_ST_REG) & bitmask )
			break;
		pds_delay_10us(10);
	}
#ifdef _DEBUG
	if (!retry) { dbgprintf(("ich_codec_rdy: timeout\n" )); }
#endif
	return retry;
}

static void ich_codec_semaphore(struct intel_card_s *card,unsigned int codec)
/////////////////////////////////////////////////////////////////////////////
{
	unsigned int retry;

	ich_codec_rdy(card,codec);

	//wait for semaphore ready (not busy) status
	for ( retry = ICH_DEFAULT_RETRY; retry; retry-- ) {
		if(!(ich_read_8(card,ICH_ACC_SEMA_REG) & ICH_CODEC_BUSY))
			break;
		pds_delay_10us(10);
	}
#ifdef _DEBUG
...
static void ich_codec_write(struct intel_card_s *card,unsigned int reg,unsigned short data)
///////////////////////////////////////////////////////////////////////////////////////////
{
...
#ifdef _DEBUG
	if ( !retry ) { dbgprintf(("ich_codec_read: timeout\n" )); }
#endif
	return data;
}

/* called by ICH_adetect();
 * inits card->dm, card->pcmout, card->virtualpagetable;
 */
...
#endif
	dbgprintf(("ich_buffer init: pagetable:%X pcmoutbuf:%X size:%d periods=%u\n",(unsigned long)card->virtualpagetable,(unsigned long)card->pcmout_buffer,card->pcmout_bufsize, card->periods));
	return 1;
}
...
static void ich_chip_init(struct intel_card_s *card)
////////////////////////////////////////////////////
{
	unsigned int cmd,retry;

	dbgprintf(("ich_chip_init: enter\n"));
	...
	ich_write_32(card, ICH_GBL_CTL_REG, cmd);
	dbgprintf(("ich_chip_init: AC97 reset type: %s\n",((cmd & ICH_GBL_CTL_AC97COLD) ? "cold":"warm")));

	for ( retry = ICH_DEFAULT_RETRY; retry; retry-- ) {
		unsigned int cntreg = ich_read_32(card,ICH_GBL_CTL_REG);
		if(!(cntreg & ICH_GBL_CTL_AC97WARM))
			break;
		pds_delay_10us(10);
	}
#ifdef _DEBUG
	if ( !retry ) { dbgprintf(("ich_chip_init: reset timeout\n" )); }
#endif
	// wait for primary codec ready status
	retry = ich_codec_rdy(card,ICH_GBL_ST_PCR);
#ifdef _DEBUG
	if ( !retry ) dbgprintf(("ich_chip_init: primary codec not ready\n"));
#endif
	...
	if ( card->device_type == DEVICE_SIS ) {
		...
		ich_write_16( card, 0x4C, ich_read_16( card, 0x4C ) | 1 ); /* unmute? */
	}

	dbgprintf(("ich_chip_init: exit\n"));
}

static void ich_chip_close(struct intel_card_s *card)
/////////////////////////////////////////////////////
{
	if(card->baseport_bm)
...
///////////////////////////////////////////////////////////////////////
{
	 ...
	 */
	ich_codec_write(card, AC97_MASTER_VOL_STEREO, 0x3F3F);
	if (ich_codec_read(card, AC97_MASTER_VOL_STEREO) != 0x3F3F) {
	...
	ich_codec_write(card, AC97_MASTER_VOL_STEREO, 0x0202);
	ich_codec_write(card, AC97_PCMOUT_VOL,        0x0202);
	ich_codec_write(card, AC97_HEADPHONE_VOL,     0x0202);
	/* v1.8: preserve the other bits of extended status reg */
	//ich_codec_write(card, AC97_EXTENDED_STATUS,AC97_EA_SPDIF);
	eastat = ich_codec_read(card, AC97_EXTENDED_STATUS);
	...
	ich_codec_write( card, AC97_EXTENDED_STATUS, eastat );
	if( ich_codec_read( card, AC97_EXTENDED_STATUS ) & AC97_EA_VRA )
		card->vra = 1;

	dbgprintf(("ich_ac97_init: end (vra:%d)\n",card->vra));
}

/*
 * called by ICH_setrate()
 */
static void ich_prepare_playback( struct intel_card_s *card, struct audioout_info_s *aui )
//////////////////////////////////////////////////////////////////////////////////////////
{
	...
	for ( retry = ICH_DEFAULT_RETRY; retry; retry-- ) {
		if(ich_read_8(card,card->sr_reg) & ICH_PO_SR_DCH)
			break;
		pds_delay_10us(1);
	}
#ifdef _DEBUG
	if (!retry) { dbgprintf(("ich_prepare_playback: dma stop timeout=%d\n",retry)); }
#endif
	// reset codec
	ich_write_8(card,ICH_PO_CR, ich_read_8(card, ICH_PO_CR) | ICH_PO_CR_RESET);
	...
	codecdata = ich_codec_read( card, AC97_SPDIF_CONTROL );
	codecdata &= AC97_SC_SPSR_MASK;
	codecdata |= spdif_rate;
	ich_codec_write( card, AC97_SPDIF_CONTROL, codecdata);
	pds_delay_10us(10);

	//set analog ac97 freq
	dbgprintf(("ich_prepare_playback: AC97 front dac freq=%d\n",aui->freq_card));
	if( card->vra ) {
		ich_codec_write( card, AC97_PCM_FRONT_DAC_RATE, aui->freq_card);
		aui->freq_card = ich_codec_read( card, AC97_PCM_FRONT_DAC_RATE);
		dbgprintf(("ich_prepare_playback: SRC used, freq now %u\n", aui->freq_card ));
		pds_delay_10us(1600);
	}
	...
	ich_write_8(card,ICH_PO_LVI, ICH_DMABUF_PERIODS - 1); /* init last valid index */
	//ich_write_8(card,ICH_PO_CIV,0); // reset current index (is ro!)

	dbgprintf(("ich_prepare playback exit\n"));
}
...
////////////////////////////////////////////////////
{
	...
	ich_codec_rdy(card,ICH_GBL_ST_PCR);
#if 0
	cmd = ich_read_8(card,ICH_PO_CR);
	cmd |= ICH_PO_CR_START;
...
///////////////////////////////////////////////////
{
	...
	ich_write_8( card, ICH_PO_CR, cmd);
#else
	ich_write_8( card, ICH_PO_CR, ich_read_8( card, ICH_PO_CR ) & ~ICH_PO_CR_START );
#endif
}
...
////////////////////////////////////////////////////////////////////////////////////
{
	...
	return tmp;
#else
	return ich_codec_read(card,reg);
#endif
}
...
{
	struct intel_card_s *card = aui->card_private_data;
	int status = ich_read_8(card,card->sr_reg);
	//dbgprintf(("ICH_IRQRoutine: SR=%X\n", status ));
	status &= ICH_PO_SR_LVBCI | ICH_PO_SR_BCIS | ICH_PO_SR_FIFO;
	if(status)
		ich_write_8(card, card->sr_reg, status); //ack
	return status != 0;
}
