...
//internal datas
static uint8_t VDMA_VMask[8];      // it's just 0 or 1
...
static uint8_t VDMA_InIO[8];       // in the middle of reading counter/addr
    ...
    if(( port >= VDMA_REG_STATUS_CMD && port <= VDMA_REG_MULTIMASK) ||
       ( port >= VDMA_REG_STATUS_CMD16 && port <= VDMA_REG_MULTIMASK16 ) ) {
        ...
        if((port >= VDMA_REG_STATUS_CMD16 && port <= VDMA_REG_MULTIMASK16)) {
            index = (port - VDMA_REG_STATUS_CMD16) / 2 + 8; /* map to VDMA_Regs[0x8-0xF] */
            base = 16; /* index base for VDMA_Regs[] access */
            channelbase = 4;
        }

        if( index == VDMA_REG_FLIPFLOP )
            VDMA_Regs[base + index] = 0;
        else if( index == VDMA_REG_MODE ) {
            int channel = byte & 0x3; //0~3
            VDMA_Modes[channelbase + channel] = byte & ~0x3;
        } else
            VDMA_Regs[base + index] = byte;
    } else if(( (int16_t)port >= VDMA_REG_CH0_ADDR && port <= VDMA_REG_CH3_COUNTER ) || /* ports 00-07? */
            ( port >= VDMA_REG_CH4_ADDR && port <= VDMA_REG_CH7_COUNTER )) { /* or ports C0-CE? */
        int channel = ( port >> 1 );
        int base = 0;

        if(( port >= VDMA_REG_CH4_ADDR && port <= VDMA_REG_CH7_COUNTER )) {
            index = ( port - VDMA_REG_CH4_ADDR ) / 2; /* map to 0-7 */
            base = 16;
            channel = ( index >> 1 ) + 4;
        }
...
////////////////////////////////
{
    ...
    if( port <= VDMA_REG_CH3_COUNTER )
        channel = (port >> 1);
    else if( port >= VDMA_REG_CH4_ADDR && port <= VDMA_REG_CH7_COUNTER )
        channel = (( port - VDMA_REG_CH4_ADDR ) >> 2 ) + 4;
    else if( port >= 0x80 && port <= 0x8F )
    ...
    if( VMDA_IS_CHANNEL_VIRTUALIZED( channel ) ) {
        ...
        if( ( (int16_t)port >= VDMA_REG_CH0_ADDR && port <= VDMA_REG_CH3_COUNTER ) ||
           ( port >= VDMA_REG_CH4_ADDR && port <= VDMA_REG_CH7_COUNTER ) ) {
            ...
            uint8_t ret;
            if( port >= VDMA_REG_CH4_ADDR && port <= VDMA_REG_CH7_COUNTER ) {
                index = ( port - VDMA_REG_CH4_ADDR ) / 2; /* [index] now 0-15 */
                base = 16;
            }
    ...
    if( port == VDMA_REG_STATUS_CMD ) {
        ...
        for( i = 0; i < 8; ++i ) {
            if( VMDA_IS_CHANNEL_VIRTUALIZED( i ) ) {
                ...
                //result |= !VDMA_Complete[i] && VDMA_CurIdx[i] != -1 && VDMA_CurIdx[i] < VDMA_GetCounter(i) ? (1<<(i+4)) : 0;
                VDMA_Complete[i] = 0; //clear on read
            }
        }
        dbgprintf(("VDMA_Read(port %X)=%02x (status cmd)\n", port, result));
    }
    return result;
}
